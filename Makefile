...
ci:
	$(call run_in_container,git config --global --add safe.directory /workspace && dart format --output=none --set-exit-if-changed . && dart run build_runner build && git diff --exit-code && flutter analyze && flutter test && flutter build web --release $(FLUTTER_DEFINES))
